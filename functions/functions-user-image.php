...
class simple_local_avatars 
{
	...
	function get_avatar( $avatar = '', $id_or_email, $width = '96', $height='96',  $default = '', $alt = false )
	{
		...
		if ( !isset($local_avatars) || empty($local_avatars) || !isset($local_avatars['full']) ) 
		{
			...
			$avatar = get_avatar( $id_or_email, $width, $height, $default );
			add_filter( 'get_avatar', 'get_simple_local_avatar', 10, 5 );
			return $avatar;
		}
		
		if ( !is_numeric($width) )		// ensure valid size
			$width = '96';

		if ( !is_numeric($height) )		// ensure valid size
			$height = '96';
		...
		// generate a new size
		if ( empty( $local_avatars[$width.'x'.$height] ) )
		{
			$upload_path = wp_upload_dir();
			$avatar_full_path = str_replace( $upload_path['baseurl'], $upload_path['basedir'], $local_avatars['full'] );
			$image_sized = image_resize( $avatar_full_path, $width, $height, true );
				
			if ( is_wp_error($image_sized) )		// deal with original being >= to original image (or lack of sizing ability)
				$local_avatars[$width.'x'.$height] = $local_avatars['full'];
			else
				$local_avatars[$width.'x'.$height] = str_replace( $upload_path['basedir'], $upload_path['baseurl'], $image_sized );	
			
			update_user_meta( $user_id, 'simple_local_avatar', $local_avatars );
		}
		elseif ( substr( $local_avatars[$width.'x'.$height], 0, 4 ) != 'http' )
			$local_avatars[$width.'x'.$height] = site_url( $local_avatars[$width.'x'.$height] );
		
		$author_class = is_author( $user_id ) ? ' current-author' : '' ;
		$size = $width.'x'.$height;
		$avatar = "<img alt='" . esc_attr($alt) . "' src='" . $local_avatars[$width.'x'.$height] . "' class='avatar avatar-{$size}{$author_class} photo' height='{$height}' width='{$width}' />";
 ...
 * @param int $width Size of the avatar image
 * @param int $height Size of the avatar image
 * @param string $default URL to a default image to use if no avatar is available
 * @param string $alt Alternate text to use in image tag. Defaults to blank
 * @return string <img> tag for the user's avatar
 */
function get_simple_local_avatar( $id_or_email, $width = '96', $height = '96', $default = '', $alt = false ) 
{
	global $simple_local_avatars;
	return $simple_local_avatars->get_avatar( '', $id_or_email, $width, $height, $default, $alt );
}
